...
<?php
if (isset($GLOBALS['m']['first_order'])) {
    $paramsForJS = "?m=" . implode(',',$GLOBALS['m']['first_order']);
}
if (isset($GLOBALS['m']['default_order'])) {
    $paramsForJS .= "," . implode(',',$GLOBALS['m']['default_order']);
}



if (isset($paramsForJS)) {
    echo '<script defer src="/load-scripts'.$paramsForJS.'&v='.rand(1,100000).'"></script>';
}
?>
