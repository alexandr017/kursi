<div class="universalrating" id="rating_{{$employee->id}}">
    ...
    <div class="universalrating-total">
        <p>
            <span class="quantity">{{$employee->rating_count}}</span> оценок, среднее <span class="rating">{{$employee->rating_value}}</span> из 5
        </p>
    </div>
</div>
...
<script>
    ...
    function handleSendRating(evt) {
        ...
        fetch(`/employee/${employeeId}/rate`, {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                let block = document.getElementById(`rating_${employeeId}`)
